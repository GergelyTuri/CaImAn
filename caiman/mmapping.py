...
#TODO: update docs/arguments, parallization
def save_memmap_from_sima(filename, base_name='Yr', resize_fact=(1, 1, 1),
    remove_init=0, idx_xy=None, order='C',xy_shifts=None,is_3D=False,
    add_to_movie=0, plane=-1):
    ...
    ----------
        ...
        resize_fact: tuple
            x,y, and z downampling factors (0.5 means downsampled by a factor 2)
        remove_init: int
            number of frames to remove at the begining of each tif file (used
            for resonant scanning images if laser in rutned on trial by trial)
        idx_xy: tuple size 2 [or 3 for 3D data]
        ...
        order: string
            whether to save the file in 'C' or 'F' order
        xy_shifts: list
            x and y shifts computed by a motion correction algorithm to be
            applied before memory mapping
    ...
    Ttot = 0
    dataset = ImagingDataset.load(filename)
    num_frames = dataset.num_frames
    #TODO
    # num_frames = 500
    channel = 1
    xfer_max = 100
    blocks = np.linspace(0, num_frames,
        np.ceil(float(num_frames)/xfer_max)+1).astype('int')
    for i in xrange(1,len(blocks)):
        print('frames {} to {}'.format(blocks[i-1], blocks[i]))

        if plane == -1:
            input_arr = np.nan_to_num(np.array(
                dataset.sequences[0][blocks[i-1]:blocks[i],:,:,:,
                    channel]).squeeze()).astype('float32')
            input_arr = np.mean(input_arr, 1)
        else:
            input_arr = np.nan_to_num(np.array(
                dataset.sequences[0][blocks[i-1]:blocks[i],plane,:,:,
                    channel]).squeeze()).astype('float32')

        #TODO: frame_rate (fr)
        Yr = cm.movie(input_arr,fr=30,start_time=0,
            file_name=os.path.split(filename)[-1], meta_data=None)
        ...
        if i == 1:
            ...
            big_mov = np.memmap(fname_tot, mode='w+', dtype=np.float32,
                                shape=(np.prod(dims), num_frames), order=order)
        else:
            big_mov = np.memmap(fname_tot, dtype=np.float32, mode='r+',
                                shape=(np.prod(dims), num_frames), order=order)
